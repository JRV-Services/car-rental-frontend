  ...
  methods: {
    async getBooking() {
      if (this.$store.state.user.roles[0] == "admin") {
        this.$setLoader();
        await dataService
          .getBooking({ _id: this.$route.params.cardId })
          .then((res) => {
            this.bookings = res.data.getBooking[0];
            let newStartDate = this.bookings.reservationDateStart;
            this.startDate = newStartDate.slice(0, 10);
            let newEndDate = this.bookings.reservationDateEnd;
            this.endDate = newEndDate.slice(0, 10);
            if (this.bookings.extensionEndDate) {
              let newreservationDate = this.bookings.extensionEndDate;
              this.extensionEndDate = newreservationDate.slice(0, 10);
            }
          });
    ...
    removeUserUrl(url) {
      ...
      if (index !== -1) {
        this.bookings.customerDocuments.splice(index, 1);
      }
    },
    ...
    removeReceiptsUrl(url) {
      ...
      if (index !== -1) {
        this.bookings.receipts.splice(index, 1);
      }
    },
    ...
    uploadUserImage(file) {
      if (file) {
        ...
        this.uploadFileToCloudinary(file).then((fileResponse) => {
          ...
          this.bookings.customerDocuments.push({ url: url, id: newId });
          this.$disableLoader();
        });
      }
    },
    ...
    uploadReceipts(file) {
      if (file) {
        ...
        this.uploadFileToCloudinary(file).then((fileResponse) => {
          ...
          this.bookings.receipts.push({ url: url, id: newId });
          this.$disableLoader();
        });
      }
    },
    ...
    async addBooking() {
      ...
      await dataService.updateBooking(this.bookings).then((res) => {
        console.log(res);
